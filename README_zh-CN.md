...
自动将漫画 ZIP 压缩包翻译为 26 种目标语言，支持 OCR 技术和可选上色功能。本服务会使用 oomol 内部的 token 进行扣费，扣费主题为 `manga-zip-translate`，目前价格为 500 页/Token, 开启着色后为两倍计量。

翻译过程会使用少量 AI 进行翻译，并记录相关费用。请注意，500 页/Token 的定价未来可能会有调整。
